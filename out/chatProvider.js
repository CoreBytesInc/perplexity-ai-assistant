...
class PerplexityCustomChatProvider {
    ...
    async handleUserMessage(message) {
        ...
        try {
            ...
            const response = await this.queryPerplexityAPI(apiKey, fullMessage, this._abortController.signal);
    ...
    async showChatHistory() {
        ...
        if (selected) {
            ...
            const userMessages = selected.session.messages.filter((m) => m.role === "user").length;
            const assistantMessages = selected.session.messages.filter((m) => m.role === "assistant").length;
            vscode.window.showInformationMessage(`Loaded chat session: ${selected.session.title} (${userMessages} questions, ${assistantMessages} responses)`);
        }
    }
    ...
    updateWebview(showTyping = false) {
        ...
        try {
            const session = this._sessions.find((s) => s.id === this._currentSessionId);
            console.log("Updating webview with session:", this._currentSessionId, "Messages:", session?.messages?.length || 0);
            this._view.webview.postMessage({
                type: "updateChat",
    ...
    async queryPerplexityAPI(apiKey, prompt, signal) {
    ...
    async handleStreamingResponse(body, signal) {
    ...
    async handleRequestAdditionalContext() {
        try {
            ...
            if (openEditors.length > 0) {
                ...
                for (const editor of openEditors) {
                    const fileName = editor.uri.path.split("/").pop() ||
            ...
            if (vscode.workspace.workspaceFolders &&
                ...
                try {
                    const workspaceFiles = await vscode.workspace.findFiles("**/*.{js,ts,py,java,cpp,c,cs,php,rb,go,rs,html,css,scss,vue,jsx,tsx,json,md,txt}", "**/node_modules/**", 20 // Limit to 20 files for performance
                    );
    ...
    async addFileToContext(filePath) {
        try {
            ...
            const existingFile = this.attachedContext.find((ctx) => ctx.type === "file" && ctx.path === filePath);
            if (!existingFile) {
    ...
    async getFileIconUri(fileUri) {
        try {
            const fileName = fileUri.path.split("/").pop() || "";
            const extension = fileName.split(".").pop()?.toLowerCase() || "";
            console.log(`Looking for icon for file: ${fileName}, extension: ${extension}`);
            ...
            let materialIconExt = allExtensions.find((ext) => ext.id === "pkief.material-icon-theme");
            if (!materialIconExt) {
                // Try alternative search methods
                materialIconExt = allExtensions.find((ext) => ext.packageJSON?.name === "material-icon-theme");
            }
            if (!materialIconExt) {
                materialIconExt = allExtensions.find((ext) => ext.packageJSON?.displayName?.includes("Material Icon Theme"));
            }
            ...
            if (materialIconExt) {
                ...
                console.log(`Extension URI: ${materialIconExt.extensionUri.toString()}`);
            ...
            <text x="8" y="12" text-anchor="middle" fill="white" font-size="10" font-family="Arial">${extension
                    ...
                    const dataUri = `data:image/svg+xml;base64,${Buffer.from(testSvg).toString("base64")}`;
            ...
            else if (lowerFileName.endsWith(".config.js") ||
            ...
            const iconPath = vscode.Uri.joinPath(materialIconExt.extensionUri, "icons", `${iconName}.svg`);
            ...
            try {
                ...
                const fallbackIconPath = vscode.Uri.joinPath(materialIconExt.extensionUri, "icons", "file.svg");
                try {
                    ...
                    console.warn(`Fallback icon also missing: ${fallbackIconPath.toString()}`);
            ...
            console.log(`Icon for ${fileName}: ${iconName}.svg -> ${webviewUri.toString()}`);
            return webviewUri.toString();
        }
        catch (error) {
    ...
    handleRemoveContext(contextId) {
        this.attachedContext = this.attachedContext.filter((ctx) => ctx.id !== contextId);
        this._view?.webview.postMessage({
            type: "contextAttached",
            context: this.attachedContext,
        });
    ...
    _getHtmlForWebview(webview) {
        ...
        const webviewPath = vscode.Uri.joinPath(this._extensionUri, "src", "chat-view");
